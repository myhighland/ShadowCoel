...
open class Manager {
    ...
    open let wormhole = MMWormhole(applicationGroupIdentifier: ShadowCoel.sharedGroupIdentifier(), optionalDirectory: "wormhole")
    ...
    open func switchVPNFromTodayWidget(_ context: NSExtensionContext) {
        if let url = URL(string: "ShadowCoel://switch") {
    ...
    func copyGEOIPData() throws {
        ...
        let toURL = ShadowCoel.sharedUrl().appendingPathComponent("GeoLite2-Country.mmdb")
    ...
    func copyTemplateData() throws {
        ...
        let toDirectoryURL = ShadowCoel.sharedUrl().appendingPathComponent("httptemplate")
    ...
    fileprivate func getDefaultConfigGroup() -> ConfigurationGroup {
        if let groupUUID = ShadowCoel.sharedUserDefaults().string(forKey: kDefaultGroupIdentifier), let group = DBUtils.get(groupUUID, type: ConfigurationGroup.self), !group.deleted {
            return group
        }else {
    ...
    open func setDefaultConfigGroup(_ id: String, name: String) {
        ...
        ShadowCoel.sharedUserDefaults().synchronize()
    }
...
extension Manager {
    ...
    func generateGeneralConfig() throws {
        let confURL = ShadowCoel.sharedGeneralConfUrl()
    ...
    func generateSocksConfig() throws {
        ...
        try socksConf.write(to: ShadowCoel.sharedSocksConfUrl(), atomically: true, encoding: String.Encoding.utf8)
    }
    
    func generateShadowsocksConfig() throws {
        let confURL = ShadowCoel.sharedProxyConfUrl()
        var content = ""
    ...
    func generateHttpProxyConfig() throws {
        let rootUrl = ShadowCoel.sharedUrl()
        ...
        let maxminddbPath = ShadowCoel.sharedUrl().appendingPathComponent("GeoLite2-Country.mmdb").path
        let userActionUrl = confDirUrl.appendingPathComponent("ShadowCoel.action")
...
//        mainConf["debug"] = mainConf["debug"] as! Int + 4096
        ...
        try mainContent.write(to: ShadowCoel.sharedHttpProxyConfUrl(), atomically: true, encoding: String.Encoding.utf8)
    ...
    fileprivate func loadAndCreateProviderManager(_ complete: @escaping (NETunnelProviderManager?, Error?) -> Void ) {
        NETunnelProviderManager.loadAllFromPreferences { [unowned self] (managers, error) -> Void in
            if let managers = managers {
                ...
                quickStartRule.connectionRules = [NEEvaluateConnectionRule(matchDomains: ["connect.ShadowCoel.com"], andAction: NEEvaluateConnectionRuleAction.connectIfNeeded)]
