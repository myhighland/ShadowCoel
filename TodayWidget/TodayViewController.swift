...
class TodayViewController: UIViewController, NCWidgetProviding, UITableViewDataSource, UITableViewDelegate, GCDAsyncSocketDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let port = ShadowCoel.sharedUserDefaults().integer(forKey: "tunnelStatusPort")
        status = port > 0
    ...
    func switchVPN() {
        if status {
            ...
            URLSession.shared.dataTask(with: URL(string: "https://on-demand.connect.ShadowCoel.com/start/")!).resume()
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            let name = ShadowCoel.sharedUserDefaults().object(forKey: kDefaultGroupName) as? String
