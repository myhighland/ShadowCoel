...
- (void)startSocksProxy:(SocksProxyCompletion)completion {
    ...
    NSString *confContent = [NSString stringWithContentsOfURL:[ShadowCoel sharedSocksConfUrl] encoding:NSUTF8StringEncoding error:nil];
...
- (void)startHttpProxy:(HttpProxyCompletion)completion {
    ...
    [NSThread detachNewThreadSelector:@selector(_startHttpProxy:) toTarget:self withObject:[ShadowCoel sharedHttpProxyConfUrl]];
}
