...
- (void)startTunnelWithOptions:(NSDictionary *)options completionHandler:(void (^)(NSError *))completionHandler {
    [self openLog];
    NSLog(@"starting ShadowCoel tunnel...");
...
- (void)setupWormhole {
    _wormhole = [[MMWormhole alloc] initWithApplicationGroupIdentifier: [ShadowCoel sharedGroupIdentifier] optionalDirectory:@"wormhole"];
...
- (void)setupStatusSocket {
    ...
    [_statusSocket performBlock:^{
        ...
        [[ShadowCoel sharedUserDefaults] setObject:@(port) forKey:@"tunnelStatusPort"];
        [[ShadowCoel sharedUserDefaults] synchronize];
    }];
}
...
- (void)startVPNWithOptions:(NSDictionary *)options completionHandler:(void (^)(NSError *error))completionHandler {
    NSString *generalConfContent = [NSString stringWithContentsOfURL:[ShadowCoel sharedGeneralConfUrl] encoding:NSUTF8StringEncoding error:nil];
...
- (void)openLog {
    NSString *logFilePath = [ShadowCoel sharedLogUrl].path;
...
- (void)stop {
    NSLog(@"stoping ShadowCoel tunnel...");
    [[ShadowCoel sharedUserDefaults] setObject:@(0) forKey:@"tunnelStatusPort"];
    [[ShadowCoel sharedUserDefaults] synchronize];
...
- (void)sleepWithCompletionHandler:(void (^)(void))completionHandler {
    NSLog(@"sleeping ShadowCoel tunnel...");
	completionHandler();
}

- (void)wake {
    NSLog(@"waking ShadowCoel tunnel...");
}
