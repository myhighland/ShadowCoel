...
class RecentRequestsVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        wormhole.listenForMessage(withIdentifier: "tunnelConnectionRecords") { [unowned self](response) in
            ...
            ShadowCoel.sharedUserDefaults().set(response as? String, forKey: kRecentRequestCachedIdentifier)
            ShadowCoel.sharedUserDefaults().synchronize()
            return
        }
        self.updateUI(ShadowCoel.sharedUserDefaults().string(forKey: kRecentRequestCachedIdentifier))
    ...
    lazy var hintLabel: UILabel = {
        let v = UILabel()
        v.text = "ShadowCoel is not connected".localized()
