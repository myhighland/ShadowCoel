...
class PushLocalChangesOperation: PSOperations.Operation {
    ...
    func addModifiedOperation(_ name: String, records: [CKRecord]) -> PSOperations.Operation {
        let op = PushLocalModifiedChangesOperation(zoneID: ShadowCoelZoneId, name: name, modifiedRecords: records)
        return op
    }

    func addDeletedOperation(_ name: String, recordIDs: [CKRecordID]) -> PSOperations.Operation {
        let op = PushLocalDeletedChangesOperation(zoneID: ShadowCoelZoneId, name: name, deletedRecordIDs: recordIDs)
        return op
    }
