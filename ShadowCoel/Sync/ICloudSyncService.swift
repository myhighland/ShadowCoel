...
class ICloudSyncService: SyncServiceProtocol {
    ...
    func sync(_ manually: Bool = false, completion: ((Error?) -> Void)?) {
        ...
        if manually {
            ...
            setZoneChangeToken(ShadowCoelZoneId, changeToken: nil)
            _ = try? DBUtils.markAll(syncd: false)
        }

        let pushLocalChangesOperation = PushLocalChangesOperation(zoneID: ShadowCoelZoneId)
        ...
        let fetchCloudChangesOperation = FetchCloudChangesOperation(zoneID: ShadowCoelZoneId)
    ...
    func subscribeNotification() {
        ...
        let subscription = CKSubscription(zoneID: ShadowCoelZoneId, subscriptionID: ShadowCoelSubscriptionId, options: CKSubscriptionOptions(rawValue: 0))
        let info = CKNotificationInfo()
        info.shouldSendContentAvailable = true
        subscription.notificationInfo = info
        ShadowCoelDB.save(subscription, completionHandler: { (sub, error) in
            if let error = error {
    ...
    func unsubscribeNotification() {
        ...
        ShadowCoelDB.delete(withSubscriptionID: ShadowCoelSubscriptionId) { (id, error) in
            if let error = error {
